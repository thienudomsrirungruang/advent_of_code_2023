...
void solve(){
    ...
    while(!q.empty()) {
        ...
        if(s.pos.first == n-1 && s.pos.second == m-1 && s.times >= 4) {
            ans = min(ans, d);
        }

        for(int i = 0; i < 4; i++) {
            if((s.pos.first != 0 || s.pos.second != 0) && i == (s.last_dir ^ 2)) continue;
            State nxts;
            ...
            if(nxts.times > 10) continue;
            if((s.pos.first != 0 || s.pos.second != 0) && (s.times < 4 && nxts.last_dir != s.last_dir)) continue;
